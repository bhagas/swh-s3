...
router.get('/insert', cek_login, function (req, res) {
  connection.query("select nama_kelurahan from master_kelurahan where deleted=0", function (err, rows, fields) {
  res.render('content-backoffice/user/insert',{kel:rows});
  })
});
...
router.get('/edit/:id', cek_login, function (req, res) {
  connection.query("select * from user where id_user='" + req.params.id + "'", function (err, rows, fields) {
    connection.query("select nama_kelurahan from master_kelurahan where deleted=0", function (err, rowss, fields) {
    if (err) throw err;
    res.render('content-backoffice/user/edit', { data: rows, kel:rowss });
    })
  })
...
router.post('/submit_insert', upload.array(), function (req, res) {
  ...
  connection.query("insert into user (username, pwd, fullname, NIP, email, telp, kel) VALUES ('" + req.body.username + "', '" + sha1(req.body.pwd) + "', '" + req.body.fullname + "', '" + req.body.NIP + "', '" + req.body.email + "', '" + req.body.telp + "', '" + req.body.kel + "')", function (err, rows, fields) {
    if (err) throw err;
    numRows = rows.affectedRows;
  })
  res.redirect('/user');
...
router.post('/submit_edit', upload.array(), function (req, res) {
  ...
  // senjata
  //console.log(req.body)
  connection.query("update user SET username='" + req.body.username + "', pwd='" + sha1(req.body.pwd) + "', fullname='" + req.body.fullname + "', NIP='" + req.body.NIP + "', email='" + req.body.email + "', telp='" + req.body.telp + "', kel='" + req.body.kel + "' WHERE id_user='" + req.body.id_user + "' ", function (err, rows, fields) {
    if (err) throw err;
    numRows = rows.affectedRows;
  })
