...
router.get('/front', function (req, res) {
  ...
  res.render('content/login', { pesan: pesan, tujuan: menuju });
});
...
router.post('/login_front',
  passport.authenticate('local', {
    failureRedirect: '/autentifikasi/front',
    failureFlash: true
  }), function (req, res) {
    res.redirect(req.user[0].tujuan)
  }
...
router.get('/logout_front', function (req, res) {
  req.logout();
  res.redirect('/autentifikasi/front');
});
...
module.exports.cek_login = function (req, res, next) {
  ...
  if (req.isAuthenticated() && req.user[0].google != 1 && req.user[0].kel == "") {
    next();
...
module.exports.cek_login_front = function (req, res, next) {

  if (req.isAuthenticated() && req.user[0].kel !="") {
    next();

  } else {
    res.redirect('/autentifikasi/front?tujuan=' + encodeURIComponent(req.originalUrl));
  }
}
