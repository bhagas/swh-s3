...
//connection.end();
 ...
 router.get('/sawah', function(req, res){
 //connection.connect();
 //console.log(req.query)
 var tambahan = "where deleted=0 and SHAPE IS NOT NULL";
 if(req.query.id_kec){
    tambahan = tambahan+" and kecamatan= '"+req.query.id_kec+"'";
 }
 connection.query("SELECT asWkt(SHAPE) as geometry, id, pemilik, jenis_lahan, status_lahan  FROM detailLahans "+tambahan, function(err, rows, fields) {
   if (err) throw err;
